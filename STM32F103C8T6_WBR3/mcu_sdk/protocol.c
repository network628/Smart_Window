...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_SPEED_ENUM, DP_TYPE_ENUM},
  {DPID_CALIBRATION, DP_TYPE_BOOL},
  {DPID_WORK_MODE, DP_TYPE_ENUM},
  {DPID_OPEN_MODE, DP_TYPE_ENUM},
  {DPID_DELAY_TIME, DP_TYPE_VALUE},
  ...
  {DPID_SWITCH_ANTI_PINCH, DP_TYPE_BOOL},
  {DPID_ANTI_PINCH_STRENGTH, DP_TYPE_ENUM},
};
...
void all_data_update(void)
{
    ...
    mcu_dp_enum_update(DPID_STATUS,当前开合状态); //枚举型数据上报;
    ...
    mcu_dp_bool_update(DPID_CHARGE_STATE,当前充电开关); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_ANTI_THEFT,当前防盗提醒); //BOOL型数据上报;
    ...
    mcu_dp_bool_update(DPID_AUTO_POWER,当前自启动开关); //BOOL型数据上报;
    ...
    mcu_dp_enum_update(DPID_SPEED_ENUM,当前速度); //枚举型数据上报;
    mcu_dp_bool_update(DPID_CALIBRATION,当前校准); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_WORK_MODE,当前模式); //枚举型数据上报;
    mcu_dp_enum_update(DPID_OPEN_MODE,当前开启模式); //枚举型数据上报;
    mcu_dp_value_update(DPID_DELAY_TIME,当前工作模式延迟); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_SWITCH_DOOR_WIN,当前门窗开关); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_SMART_WEATHER,当前智能天气); //枚举型数据上报;
    mcu_dp_enum_update(DPID_WIND_SPEED_STATE,当前风速状态); //枚举型数据上报;
    mcu_dp_value_update(DPID_TEMP_CURRENT,当前当前温度); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_SWITCH_SENSOR,当前传感器开关); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_SWITCH_ANTI_PINCH,当前防夹开关); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_ANTI_PINCH_STRENGTH,当前放夹力度); //枚举型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_charge_state_handle
功能描述 : 针对DPID_CHARGE_STATE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_charge_state_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    charge_state = mcu_get_dp_download_bool(value,length);
    if(charge_state == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_CHARGE_STATE,charge_state);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_anti_theft_handle
功能描述 : 针对DPID_ANTI_THEFT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_anti_theft_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char anti_theft;
    
    anti_theft = mcu_get_dp_download_bool(value,length);
    if(anti_theft == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_ANTI_THEFT,anti_theft);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_speed_enum_handle
功能描述 : 针对DPID_SPEED_ENUM的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_speed_enum_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char speed_enum;
    
    speed_enum = mcu_get_dp_download_enum(value,length);
    switch(speed_enum) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_SPEED_ENUM, speed_enum);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_open_mode_handle
功能描述 : 针对DPID_OPEN_MODE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_open_mode_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char open_mode;
    
    open_mode = mcu_get_dp_download_enum(value,length);
    switch(open_mode) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_OPEN_MODE, open_mode);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_delay_time_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long delay_time;
    
    delay_time = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_DELAY_TIME,delay_time);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
static unsigned char dp_download_switch_door_win_handle(const unsigned char value[], unsigned short length)
{
    ...
    if(switch_door_win == 0) {
       FlashBuffer.power_switch = 0;//开关 关
			 LED1 = 0;LED2 = 0;LED3 = 0;LED4 = 0;//0
			 Open_Window(2);   //0-停止  1-开窗  2-关窗
			 printf("\r\n---FlashBuffer.power_switch(%d)---\r\n",FlashBuffer.power_switch);
    }else {
       FlashBuffer.power_switch = 1;//开关 开
			 LED1 = 1;LED2 = 1;LED3 = 1;LED4 = 1;//1
...
/*****************************************************************************
函数名称 : dp_download_switch_anti_pinch_handle
功能描述 : 针对DPID_SWITCH_ANTI_PINCH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_switch_anti_pinch_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char switch_anti_pinch;
    
    switch_anti_pinch = mcu_get_dp_download_bool(value,length);
    if(switch_anti_pinch == 0) {
        //开关关
    }else {
        //开关开
    }
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_SWITCH_ANTI_PINCH,switch_anti_pinch);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_anti_pinch_strength_handle
功能描述 : 针对DPID_ANTI_PINCH_STRENGTH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_anti_pinch_strength_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char anti_pinch_strength;
    
    anti_pinch_strength = mcu_get_dp_download_enum(value,length);
    switch(anti_pinch_strength) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_ANTI_PINCH_STRENGTH, anti_pinch_strength);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        break;
        case DPID_CHARGE_STATE:
            //充电开关处理函数
            ret = dp_download_charge_state_handle(value,length);
        break;
        case DPID_ANTI_THEFT:
            //防盗提醒处理函数
            ret = dp_download_anti_theft_handle(value,length);
        break;
        ...
        break;
        case DPID_SPEED_ENUM:
            //速度处理函数
            ret = dp_download_speed_enum_handle(value,length);
        break;
        ...
        case DPID_OPEN_MODE:
            //开启模式处理函数
            ret = dp_download_open_mode_handle(value,length);
        break;
        case DPID_DELAY_TIME:
            //工作模式延迟处理函数
            ret = dp_download_delay_time_handle(value,length);
        break;
        ...
        break;
        case DPID_SWITCH_ANTI_PINCH:
            //防夹开关处理函数
            ret = dp_download_switch_anti_pinch_handle(value,length);
        break;
        case DPID_ANTI_PINCH_STRENGTH:
            //放夹力度处理函数
            ret = dp_download_anti_pinch_strength_handle(value,length);
        break;
